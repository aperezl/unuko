    ...
    >
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6 lg:px-8">
        ...
        <div className="hidden md:flex items-center space-x-8 text-sm font-medium text-slate-400">
          <a href="/#features" className="hover:text-white transition-colors duration-300">{dict.navbar.features}</a>
          <a href="/#stack" className="hover:text-white transition-colors duration-300">{dict.navbar.ecosystem}</a>
          <a href="/#usecases" className="hover:text-white transition-colors duration-300">{dict.navbar.usecases}</a>
          <Link href="/docs" className="hover:text-white transition-colors duration-300">{dict.navbar.docs}</Link>
        </div>
        ...
        <div className="flex items-center gap-4">
          <Link href="/docs" className="hidden sm:inline-flex text-sm font-medium text-slate-400 hover:text-white transition-colors duration-300 px-3 py-2">
            {dict.navbar.startDoc}
          </Link>

          {/* Language Switcher */}
          <div className="flex items-center gap-1 border border-white/10 bg-white/5 rounded-lg p-0.5 text-xs font-semibold select-none backdrop-blur-md">
            <button
              onClick={() => setLanguage("es")}
              className={`px-2 py-1 rounded-md transition-all duration-300 cursor-pointer ${language === "es" ? "bg-indigo-600 text-white shadow-sm" : "text-slate-400 hover:text-white"}`}
            >
              ES
            </button>
            <button
              onClick={() => setLanguage("en")}
              className={`px-2 py-1 rounded-md transition-all duration-300 cursor-pointer ${language === "en" ? "bg-indigo-600 text-white shadow-sm" : "text-slate-400 hover:text-white"}`}
            >
              EN
            </button>
          </div>
